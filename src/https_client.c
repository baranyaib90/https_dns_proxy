...
static int closesocket_callback(void __attribute__((unused)) *clientp, curl_socket_t sock)
{
  ...
  if (client->connections <= 0) {
    ILOG("Client reset timer cancelled");
    ev_timer_stop(client->loop, &client->client_reset_timer);
  }
                                            ...
                                            CURLcode curl_result_code)
{
  ...
  switch (curl_result_code) {
    ...
    case CURLE_OPERATION_TIMEDOUT:
      if (!ev_is_active(&client->client_reset_timer)) {
        ILOG_REQ("Client reset timer started");
        ev_timer_again(client->loop, &client->client_reset_timer);
      }
      __attribute__((fallthrough));
    default:
...
static void client_reset_timer_cb(struct ev_loop __attribute__((unused)) *loop,
...
void https_client_init(https_client_t *c, options_t *opt,
  ...
  ev_init(&c->client_reset_timer, client_reset_timer_cb);
  c->client_reset_timer.repeat = 10.0;
  c->client_reset_timer.data = c;
...
void https_client_cleanup(https_client_t *c) {
  ...
  ev_timer_stop(c->loop, &c->client_reset_timer);
}
