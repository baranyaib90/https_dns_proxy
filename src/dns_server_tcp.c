...
enum {
  LISTEN_BACKLOG = 5,
  MAX_CLIENTS = 5, // TODO: change to 20
  IDLE_TIMEOUT_S = 30, // TODO: change to 120, // two minutes according to RFC1035 4.2.2
  SEND_RETRY_DELAY_US = 500,  // 0.0005 sec
  MIN_DNS_LENGTH = 12, // RFC1035 4.1.1 header size
};

// TODO:
// - logs to contain socket reference

static int get_tcp_listen_sock(struct addrinfo *listen_addrinfo);
static void accept_cb(struct ev_loop *loop, ev_io *w, int revents);
static void read_cb(struct ev_loop *loop, ev_io *w, int revents);
static int get_dns_request(struct tcp_client_s *client, char ** dns_req, uint16_t * req_size);
...
static void accept_cb(struct ev_loop __attribute__((unused)) *loop,
  ...
  DLOG("Added client: %d", client->sock);
}
...
static void read_cb(struct ev_loop __attribute__((unused)) *loop,
  ...
  if (len <= 0) {
    if (len == 0 || errno == ECONNRESET) {
      ...
      WLOG("Read error: %s", strerror(errno));
    }
    remove_client(client);
    return;
  }

  uint32_t free_space = client->input_buffer_size - client->input_buffer_used;
  DLOG("Received %d byte, free: %u", len, free_space);
  if (free_space < len) {
    uint32_t needed_space = (uint32_t)len - free_space;
    DLOG("Realloc buffer: %u + %u", client->input_buffer_size, needed_space);
  ...
  client->input_buffer_used += (uint32_t)len;
  ...
  while (get_dns_request(client, &dns_req, &req_size)) {
    if (req_size < MIN_DNS_LENGTH) {
      WLOG("Malformed request received, too short: %u", req_size);
      free(dns_req);
      remove_client(client);
      return;
    }
    
    d->cb(d, d->cb_data, (struct sockaddr*)&client->raddr, dns_req, req_size);
    request_received = 1;
  }
...
static void timer_cb(struct ev_loop __attribute__((unused)) *loop,
  ...
  DLOG("TCP client timeouted");
  remove_client((struct tcp_client_s *)w->data);
}

static void remove_client(struct tcp_client_s * client) {
  dns_server_tcp_t *d = client->d;

  DLOG("Removing client: %d", client->sock);
    ...
    struct sockaddr *raddr, char *resp, size_t resp_len)
{
  ...
  if (len != sizeof(uint16_t)) {
    WLOG("Send error: %s, len: %d", strerror(errno), len);
    remove_client(client);
    return;
  }
  ...
  for (uint8_t i = 0; i < UINT8_MAX; ++i)  // endless loop guard
  {
    ...
    if (len < 0) {
      if (errno != EAGAIN && errno != EWOULDBLOCK) {
        WLOG("Send error: %s", strerror(errno));
        remove_client(client);
        return;
      }
    }
